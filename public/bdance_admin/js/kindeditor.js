...
_extend(KCmd, {
	...
	insertimage : function(url, title, width, height, border, align) {
		...
		var html = '<a href="' + _escape(url) + '" target="_blank"><img src="' + _escape(url) + '" style="max-width: 500px;" data-ke-src="' + _escape(url) + '" ';
		if (width) {
...
_extend(KWidget, {
	init : function(options) {
		...
		if (options.autoScroll) {
			if (_IE && _V < 7 || _QUIRKS || _CHROME) {
				var scrollPos = _getScrollPos();
				
				if(_CHROME)
				{
					self.div.css('top', '184px');
