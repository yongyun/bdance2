$(document).ready(function() {
    ...
    function createDlg(imagesListUrl, uploadUrl, data) {
        ...
        $.getJSON(imagesListUrl, function(jsonData){
            ...
            ' <li class="active"><a href="#tab2" data-toggle="tab">上傳圖片</a></li>'+	
            '</ul>'+
			...
			*/
            ...
            '<div class="tab-pane span3" id="tab2">'+
            ...
            '<tr><td>請選擇圖片:</td></tr>' +
            '<tr><td> ' +
            '<input id="imageName" name="imageName" type="file" /> </td></tr>' +
            '<tr><td>或輸入圖片網址:</td></tr>' +
            '<tr><td><input type="text" size="40" value="" /></td></tr>' +
            '</table></form><input type="button" id="uploadBtn" value="送出"><br /><br />'+		
            '</div>'+	
            '</div>'+			
            '</div>';       
            $(popup).html(dlgBody);
	...
	// Url for file upload 
	$.cleditor.buttons.image.uploadUrl = 'editor_upload_image.php';
	
	// Url for images list
	$.cleditor.buttons.image.imageListUrl = 'image-list.json';

	$.cleditor.defaultOptions.width = 750;
